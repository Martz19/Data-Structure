...
void IntSLList<T>::sortInsert(T val){
	if(head==0)	{addToTail(val);}
	else {
		IntSLLNode<T> *tmp, *tmp2 = NULL;
		tmp = head;

		while((val.getLast()<tmp->getInfo().getLast()) && (tmp != tail)){
			tmp2=tmp;
			tmp=tmp->getNext();}
	if((tmp == tail) && (val.getLast() <tmp->getInfo().getLast())) {addToTail(val);}
	else {
		if((tmp==head) && (val.getLast() >tmp->getInfo().getLast())){
			addToHead(val);
		}
...
void IntSLList<T>::printList(){
	...
	cout << prtNode;
	while(prtNode != 0) {
		cout<<prtNode->getInfo().getLast()<<"  ";
		cout << prtNode->getInfo().getFirst() << "  ";
